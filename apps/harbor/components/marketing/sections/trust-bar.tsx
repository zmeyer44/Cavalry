    ...
    <section className="bg-white pb-16 pt-6 md:pb-24 md:pt-10">
      <div className="mx-auto max-w-[1120px] px-6 md:px-10">
        <p className="mb-10 text-center font-mono text-[11px] uppercase tracking-[0.18em] text-stone-500">
          Built on the AI platforms your operations team can trust
        </p>
        <ul role="list" className="mx-auto grid max-w-[960px] grid-cols-2 gap-2.5 md:grid-cols-4">
          {slots.map((slot, i) => (
            <li
              key={i}
              className={
...
function LogoLayer({
  ...
  const animationClass =
    phase === 'in' ? 'harbor-slot-in' : phase === 'out' ? 'harbor-slot-out' : '';
  return (
